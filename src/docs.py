...
def search(query=None, version=Config.DEFAULT_DJANGO_VERSION, limit=Config.RESULT_COUNT):
    if query:
        results = index.search(query, {'filters': 'version=' + version,
                                       'page': 0,
                                       'hitsPerPage': limit})
...
def main(wf):
    ...
    results = wf.cached_data(key, functools.partial(search, query, version),
                             max_age=Config.CACHE_MAX_AGE)

    log.debug('{} results for {!r}, version {!r}'.format(len(results),
                                                         query,
                                                         version))
    # Show results
    if not results:
