...
use alfred_django_docs::app::{google_fallback_item, items_from_results, placeholder_item};
use alfred_django_docs::models::SearchResult;
use alfred_django_docs::services::AlgoliaSearch;
use alfred_workflow_rs::{Icon, Item, RenderOptions, Updater, UserConfiguration, Workflow};
use anyhow::{Result, anyhow};

use crate::cli::Cli;

const GITHUB_REPOSITORY_URL: &str = "https://github.com/techouse/alfred-django-docs";
...
{
    let query = cli.normalized_query(&settings.django_version);
    if cli.verbose {
    ...
    let results = search(&query, &settings.django_version)?;
    if results.is_empty() {
...
fn configure_cache(workflow: &mut Workflow, query: &str, settings: &WorkflowSettings) {
    if settings.use_alfred_cache {
        ...
        workflow.set_cache_key(Some(file_cache_key(query, &settings.django_version)));
        workflow.set_max_cache_entries(settings.file_cache_max_entries);
    }
    workflow.set_cache_time_to_live(settings.cache_ttl);
}

fn file_cache_key(query: &str, django_version: &str) -> String {
    format!("{query}_{django_version}")
}
...
fn workflow_settings_from_defaults(
    ...
    let django_version = select_value(defaults, "django_version")
        .ok_or_else(|| anyhow!("django_version not set!"))?;

    Ok(WorkflowSettings {
        django_version: django_version.to_owned(),
