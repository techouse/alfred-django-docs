...
/// Configuration required to query the Django Algolia index.
...
/// Synchronous client for the Django documentation Algolia index.
#[derive(Clone, Debug)]
...
impl AlgoliaSearch {
    ...
    /// Searches the selected Django version and returns ranked hits.
    ...
    fn request_host(&self, endpoint: &Url, body: &str, remaining: Duration) -> AttemptResult {
        ...
        response_reader
            ...
            .map_err(|error| {
                classify_response_body_error(endpoint, status, ureq::Error::from(error))
            })?;
        if response_body.len() as u64 > MAX_RESPONSE_BYTES {
            let error = anyhow!(
                "Algolia response from {endpoint} with HTTP status {status} exceeds {MAX_RESPONSE_BYTES} bytes"
            );
        ...
        let response_body = match String::from_utf8(response_body) {
            ...
            Err(error) => {
                let error = anyhow!(
                    "Algolia response from {endpoint} with HTTP status {status} is not valid UTF-8: {error}"
                );
                return Err(if (500..=599).contains(&status) {
                    AttemptFailure::Retryable(error)
                } else {
            ...
            let response: SearchResponse =
                serde_json::from_str(&response_body).map_err(|error| {
                    AttemptFailure::Terminal(anyhow!(
                        "failed to deserialize Algolia response from {endpoint} with HTTP status {status}: {error}; response body: {response_body}"
                    ))
                })?;
    ...
    fn request_body(&self, query: &str, version: &str) -> Result<String> {
        Ok(serde_json::to_string(&SearchRequest {
            query,
            facet_filters: [format!("version:{}", version.replace('v', ""))],
...
fn classify_response_body_error(endpoint: &Url, status: u16, error: ureq::Error) -> AttemptFailure {
    let message = || {
        anyhow!(
            "Algolia read response body failed for {endpoint} with HTTP status {status}: {error}"
        )
    };
    ...
    if (400..=499).contains(&status) {
        ...
        classify_ureq_error(endpoint, "read response body", error)
    }
}
