...
fn endpoint_uses_single_index_search_route() -> Result<()> {
    ...
    assert_eq!(
        ...
        "http://127.0.0.1:8080/api/1/indexes/django/query"
    );
    Ok(())
}
...
#[test]
fn request_body_uses_django_twenty_hit_contract() -> Result<()> {
    let client = AlgoliaSearch::with_base_url(config(), Url::parse("http://localhost/")?)?;
    let body: Value = serde_json::from_str(&client.request_body("request body", "v5.2")?)?;

    assert_eq!(
        body,
        json!({
            "query": "request body",
            "facetFilters": ["version:5.2"],
            "attributesToRetrieve": [
                "categories", "content", "id", "permalink", "title", "version"
            ],
            "page": 0,
            "hitsPerPage": 20
        })
...
fn query_retries_retryable_failures_in_host_order() -> Result<()> {
    ...
    let hits = client.query_with("request", "v5.2", |endpoint, _, _| {
    ...
    assert_eq!(
        endpoints,
        vec![
            "http://first.test/1/indexes/django/query",
            "http://second.test/1/indexes/django/query",
            "http://third.test/1/indexes/django/query",
        ]
    );
    Ok(())
}
...
fn query_reports_all_retryable_host_failures() -> Result<()> {
    ...
    let error = client
        .query_with("request", "v5.2", |endpoint, _, _| {
            attempts += 1;
            Err(AttemptFailure::Retryable(anyhow::anyhow!(
                "{endpoint} failed"
            )))
    ...
    assert!(message.contains("first.test/1/indexes/django/query failed"));
    assert!(message.contains("second.test/1/indexes/django/query failed"));
    Ok(())
}
...
fn query_uses_one_shared_timeout_budget() -> Result<()> {
    ...
    let _ = client.query_with("request", "v5.2", |_, _, remaining| {
...
fn query_falls_back_after_5xx_response() -> Result<()> {
    ...
    let hits = client.query("request", "v5.2")?;
...
#[test]
fn query_stops_on_malformed_4xx_gzip_body() -> Result<()> {
    let (url, server) = serve_once_with_body(401, b"not-gzip".to_vec(), Some("gzip"))?;
    let client = AlgoliaSearch::with_read_hosts_and_agent(config(), vec![url], no_proxy_agent())?;

    let error = client
        .query("request", "v5.2")
        .expect_err("malformed 4xx response bodies must be terminal");
    server
        .join()
...
#[test]
fn query_falls_back_after_malformed_5xx_gzip_body() -> Result<()> {
...
#[test]
fn query_stops_on_malformed_successful_json() -> Result<()> {
    let (url, server) = serve_once(200, "not-json")?;
    let client = AlgoliaSearch::with_read_hosts_and_agent(config(), vec![url], no_proxy_agent())?;

    let error = client
        .query("request", "v5.2")
        .expect_err("malformed successful JSON must be terminal");
    server
        .join()
        .map_err(|_| anyhow::anyhow!("server thread panicked"))?;
...
#[test]
fn query_stops_on_invalid_successful_utf8() -> Result<()> {
    let (url, server) = serve_once_with_body(200, vec![0xff], None)?;
    let client = AlgoliaSearch::with_read_hosts_and_agent(config(), vec![url], no_proxy_agent())?;

    let error = client
        .query("request", "v5.2")
        .expect_err("invalid successful response bytes must be terminal");
    server
        .join()
        .map_err(|_| anyhow::anyhow!("server thread panicked"))?;
