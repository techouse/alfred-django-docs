...
fn parse_reads_query_and_flags() -> Result<()> {
    ...
    assert_eq!(
        cli,
        Cli {
            query: "  5.2   Request Body ".to_owned(),
            verbose: true,
            update: true,
        }
    );
...
fn normalized_query_removes_every_selected_version_token() {
    let cli = Cli {
        query: "  v5.2   Request Body v5.2 ".to_owned(),
        ..Cli::default()
    };

    assert_eq!(cli.normalized_query("v5.2"), "request body");
}
...
#[test]
fn parse_rejects_query_followed_by_valid_short_cluster() {
    for option in ["-vu", "-vq", "-uvq"] {
        let error = Cli::parse(["-q".to_owned(), option.to_owned()])
            .expect_err("recognized short clusters are not query values");
...
#[test]
fn parse_rejects_long_query_followed_by_valid_short_cluster() {
    let error = Cli::parse(["--query".to_owned(), "-vu".to_owned()])
        .expect_err("recognized short clusters are not query values");
...
fn parse_accepts_attached_short_query_value() -> Result<()> {
    let cli = Cli::parse(["-qrequest".to_owned()])?;

    assert_eq!(cli.query, "request");
    Ok(())
}
...
fn parse_accepts_collapsed_flags_with_attached_query() -> Result<()> {
    let cli = Cli::parse(["-vuqrequest".to_owned()])?;
...
fn parse_accepts_collapsed_flags_with_separated_query() -> Result<()> {
    let cli = Cli::parse(["-vq".to_owned(), "request".to_owned()])?;
