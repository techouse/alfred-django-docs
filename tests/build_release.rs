...
impl Fixture {
    fn new() -> Result<Self> {
        ...
        write_executable(
            &fake_bin.join("cargo"),
            "#!/usr/bin/env bash\nset -eu\nprintf '%s\\n' \"${ALGOLIA_APPLICATION_ID-}\" \"${ALGOLIA_SEARCH_ONLY_API_KEY-}\" \"${ALGOLIA_SEARCH_INDEX-}\" \"${UNRELATED_DOTENV-}\" > \"$FAKE_CARGO_RECORD\"\n",
        )?;
